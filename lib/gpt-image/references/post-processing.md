...
Install ImageMagick if not present: `brew install imagemagick` (macOS) or
...
### Cropping GPT Image output to non-native ratios

GPT Image 2 produces three native sizes: 1024x1024, 1024x1536, 1536x1024. For
other ratios, generate at the closest native size and crop:

```bash
# 1536x1024 → 16:9 (1536x864)
magick input.png -resize 1536x864^ -gravity center -extent 1536x864 output_16x9.png

# 1536x1024 → 21:9 cinematic (1536x659)
magick input.png -gravity center -crop 1536x659+0+0 +repage output_21x9.png

# 1024x1536 → 9:16 mobile (864x1536)
magick input.png -resize 864x1536^ -gravity center -extent 864x1536 output_9x16.png

# 1536x1024 → 4:1 banner (1536x384)
magick input.png -gravity center -crop 1536x384+0+0 +repage output_4x1.png
```

### Background Removal (Transparency)

GPT Image 2 cannot produce transparent PNGs directly. Generate on a clean
solid background, then key it out:
...
for f in ~/Documents/gpt_image_generated/*.png; do
...
GPT Image 2 cannot generate transparent backgrounds. For complex subjects
where simple white-background keying doesn't work, use a green screen:
