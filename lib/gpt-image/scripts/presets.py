#!/usr/bin/env python3
"""GPT Image Claude -- Brand/Style Presets
...
PRESETS_DIR = Path.home() / ".gpt-image" / "presets"
...
def cmd_create(args):
    ...
    if path.exists() and not args.force:
        print(f"Error: Preset '{args.name}' already exists. Use a different name or --force.", file=sys.stderr)
        sys.exit(1)
    ...
    preset = {
        ...
        "mood": args.mood or "",
        "default_size": args.size or "1536x1024",
        "default_quality": args.quality or "high",
    }
...
def main():
    parser = argparse.ArgumentParser(description="GPT Image Claude Brand/Style Presets")
    ...
    p_create.add_argument("--size", default="1536x1024", help="Default size")
    p_create.add_argument("--quality", default="high", help="Default quality")
